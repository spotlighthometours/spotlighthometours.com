...
var packageObj = {};
var packageClicked = false;
function showPackage(obj){
	$(".package-price, .packages td").removeClass('selected');
	$(obj).addClass('selected');
	$(obj).parent().addClass('selected');
	checkoutPrice = $(obj).data("price");
	$(".order-info .price").html("$"+checkoutPrice);
	$(".order-info .membership").html($(obj).data("choice"));
	checkoutChoice = $(obj).data("choice");
	$(".order-info, .order-info-bg").fadeIn('slow');
	checkoutType = $(obj).data("type");
	checkoutMembershipID = $(obj).data("membershipid");
	...
	_gaq.push(['_trackEvent','package','clicked',$(obj).data("choice")]);
	fbq('trackCustom', 'Package Selected V3', {
			package: $(obj).data("choice")
	});
}
function hideOrderInfo(){
	$(".order-info, .order-info-bg").fadeOut('slow');
}
$(document).ready(
	function() {	
		$('.tooltip').tooltipster({
	...
	try {
		...
		if(errors.length > 0) {
			...
			if($("#agree").is(':checked')){
				...
				outputError("order-msg", "Please agree to the terms and conditions to proceed.");
				return false;
			}
		}
	} catch(err) {
...
function submitOrder(){
	_gaq.push(['_trackEvent','order','submitted',"Membership ID: "+order.membershipid]);
	if(!ValidateCC()){
		return false;
	}
	var params = '';
...
function OrderSuccess(orderid){
	try {
		fbq('trackCustom', 'Package Ordered V3', {
			package: checkoutChoice
		});
...
function showDiscount(){
	...
	if(parseInt(discount)>0){
		$(".order-info .discount-total").html("-$"+discount);
		$(".order-info .total").html("$"+total);
		$(".order-info .coupon-discount").slideDown('slow');
	}else{
		$(".order-info .coupon-discount").css("display", "none");
	}
}

function choseAgentBroker(){
	_gaq.push(['_trackEvent','request demo','clicked',"choose agent/brokerage"]);
	var selectQuote = ' \
<div class="quote-form login-panel" id="agentBroker" style="text-align:center"> \
	<div style="margin-top:-35px"><img src="images/Agent22.png" alt="Agent Quote"  onmouseover="" style="cursor: pointer;" onclick="getAgentQuoteForm()"></div><br />\
	<img src="images/Brokerage22.png" alt="Mountain View"  onmouseover="" style="cursor: pointer;" onclick="getBrokerQuoteForm()"> \
<div class="clear"></div> \
</div>';
	ShowPopUp('',selectQuote);
}

function getBrokerQuoteForm(){
	_gaq.push(['_trackEvent','request demo','clicked',"brokerage"]);
	var quoteForm = ' \
	<div class="quote-form login-panel"><p>Simply fill out the brief form below and one of Spotlight\'s team members will contact you right away.</p><br/> \
	...
	<div align="left"><div class="button_new button_blue button_mid" onClick="requestQuote()"><div class="curve curve_left"></div><span class="button_caption">Request a Demo</span><div class="curve curve_right"></div></div> \
	</div> \
	<div class="clear"></div> \
	</div>';
...
function getAgentQuoteForm(){
	_gaq.push(['_trackEvent','request demo','clicked',"agent"]);
	var quoteForm = ' \
	<div class="quote-form login-panel"><p>Simply fill out the brief form below and one of Spotlight\'s team members will contact you right away.</p><br/> \
	<div id="brokerageSignupMsg"></div> \
	<div class="form_line"> \
	...
	<div class="form_line" style="display:none"> \
	  <div class="input_line w_lg"> \
		<div class="input_title" >Number agents</div> \
		<input id="num_agents" name="num_agents" onfocus="ToggleInputInfo(this, 1);" type="number" value="0" /> \
	  </div> \
	</div> \
	<div class="loginButtons" style="width:550px;"> \
	<div align="left"><div class="button_new button_blue button_mid" onClick="requestQuote()"><div class="curve curve_left"></div><span class="button_caption">Request a Demo</span><div class="curve curve_right"></div></div> \
...
function requestQuote(){
	_gaq.push(['_trackEvent','request demo','submitted',"brokerage/agent"]);
	if(validateQuoteRequest()){
		...
		fbq('trackCustom', 'Quote Requested V3', {
			brokerage: brokerage,
			name: contactname,
			phone: phone,
			email: email
...
function logThemIn(){
	...
	ajaxQuery(url, params, 'showOrderform');
}

function showOrderform(){
	$("#userid").val(response);
	if(packageClicked){
		//$(packageObj).trigger("click");
		showPackage(packageObj);
		HidePopUp();
	}
	/*//call funtion show order
	window.location = "concierge-checkout.php";
	//$('.signup-form').hide();
	HidePopUp(loginSignup);
	//orderForm();
	$('html, body').animate({
