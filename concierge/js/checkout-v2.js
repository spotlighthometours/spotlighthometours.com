...
function showPackage(obj){
	...
	checkoutPrice = $(obj).html();
	//alert(checkoutPrice);
	$("#summary #order-total span").html(checkoutPrice);
	$("#summary #subscription-choice").html($(obj).data("choice"));
	checkoutChoice = $(obj).data("choice");
	$("#billing").show();
	$("#summary").show();
	$('html, body').animate({
		scrollTop: $("#concierge-line2").offset().top
	}, 500);
	...
	fbq('trackCustom', 'Package Selected V2', {
...
function OrderSuccess(orderid){
	try {
		fbq('trackCustom', 'Package Ordered V2', {
...
function requestQuote(){
	...
	if(validateQuoteRequest()){
		...
		fbq('trackCustom', 'Quote Requested V2', {
			brokerage: brokerage,
			name: contactname,
			phone: phone,
			email: email
