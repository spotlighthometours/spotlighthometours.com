...
//=======================================================================
	...
	// Redirect to Social Hub main page if already a member!
	if($socialhub->isActive($userid)){
		header('Location: https://www.spotlighthometours.com/users/new/social-hub.php');
	}
	...
	// Set brokerage ID by lookup on users table by userID. Also set express user in session
	$query = 'SELECT BrokerageID, expressUser FROM `users` WHERE userID='.$userid;
...
//=======================================================================
		...
		$header = '
			...
			<link rel="stylesheet" media="all" href="socialhub_checkout.css?randIt='.$randIt.'">
			...
			<script type="text/javascript" src="socialhub_checkout.js?randIt='.$randIt.'"></script>
		';
...
//		{
			...
			$socialHub = array();
			$memberships = $m->getMemberships();

            $socialHub = $memberships[2]; //2 is the social hub membership/product
            $price = $m->getMembershipAdjustedPrice($socialHub['id'], $userid);
			...
			$header .= '
                <script>
                    window.onload = function(){
                        ...
                        order.membershiptype = "'.$socialHub['name'] .'";
                        //PopulateOrder();
                        //console.log(order);
                    }
                </script>
						...
						<div id="checkout_table" >
							<div class="pr_frame" >
                                <table class="pricing" cellpadding="0" cellspacing="0" border="0" nowrap>
                                    ...
                                    <tr>
                                        <td><?php echo  $socialHub['name']; ?></td>
                                        <td>1</td>
                                        <td class="social_hub_coupon"></td>
...
?>
					...
					<div class="form_line" >
						<div class="form_line" >
							<div class="button_new button_blue button_mid" id="subOrderBtn" onclick="ValidateSocialHub();">
								<div class="curve curve_left" ></div>
