...
//All $_POST info is being provided from the socialhub_checkout.js file.
switch( $_GET['view'] ) //check the $_GET for what to do. NEED TO refactor some day
{
    ...
    case 'submitOrder':
       ...
       //need to add in some validation that the session exists
        ...
        //echo out the response for socialhub_chckout.js
...
function InsertReoccuring($tourid, $rec) {
    ...
    if ( $stmt = $dbh->prepare($query) )
    {
        ...
        try {
            ...
            WriteLog("socialhub_functions.php", $e->getMessage());
        }

    }
}

function Email($userid, $postParams)
{
    global $dbh;
    //$body = table($order);
    ...
    //$mail->AddAddress('kyle@spotlighthometours.com');


    $subject = "Spotlight Home Tours Social Hub";
    ...
    if( !$mail->Send() ) 
    {   
        WriteLog("socialhub_functions", "Mailer Error: " . $mail->ErrorInfo);
    }
