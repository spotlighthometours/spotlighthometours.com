...
**********************************************************************************************/
    ...
    $ttypes = new tourtypes;
    $tour = array();
    $tour['tourTypeID'] = intval($_REQUEST['tourTypeId']);
    $price = number_format($_REQUEST['price'],2,'.',',');
    $cleanPrice = preg_replace("|[^\.0-9]+|","",$_REQUEST['price']);
    $ttypes->load($tour['tourTypeID']);

    $tour = $db->select("tourtypes","tourTypeID=" . intval($_REQUEST['tourTypeId']))[0];
	...
	if (isset($tour['DemoLinkLink']) && strlen($tour['DemoLinkLink']) > 0) {
		...
		<div class="tourpricing">
			<div class="tourprice" >$' .$price . '</div>
			<input id="tour' . $tour['tourTypeID'] . 'price" type="hidden" name="tour' . $tour['tourTypeID'] . 'price" value=' . $cleanPrice . '>
