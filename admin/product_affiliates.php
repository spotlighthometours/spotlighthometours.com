<?PHP
/*
 * Admin Product Affiliates Control File
 */
...
// Get blacklisted affiliates for this product
$blackListedAffiliates = $blacklist->getBlacklistedCategories('product', $_REQUEST['id'], 'affiliate');
...
$affiliatePricing = $pricing->getPricing('product', $_REQUEST['id'], 'affiliate');
...
<script>
	...
	function addSelection(id){
		query = "action=add&id="+id+"&productID=<?PHP echo $_GET['id']; ?>";
	...
	function removeSelection(id){
		query = "action=remove&id="+id+"&productID=<?PHP echo $_GET['id']; ?>";
		var url = "../repository_queries/admin_affiliate_updateproducts.php?"+query;
		params = "";
		ajaxQuery(url, params, 'nothing');
	}
	...
	function selectAll(){
		var proceed = confirm("Are you sure you want to add this product to all affiliates?");
		if(proceed){
			$('input[type=checkbox]').each(function () {
				$(this).parent().show();
				if(!this.checked){
			...
			alert('Adding all Affiliate to this product!');
		}
	}
	
	function deselectAll(){
		var proceed = confirm("Are you sure you want to remove all affiliates from this product?");
		if(proceed){
			$('input[type=checkbox]').each(function () {
				$(this).parent().show();
				if(this.checked){
			...
			alert('Removing all Affiliates from this product!');
		}
	}
	...
	function updatePrice(element){
		...
		if(element.value==""){
			...
			if(element.value.search(strPattern)== -1){
				...
				query = "action=add&id="+id+"&productID=<?PHP echo $_GET['id']; ?>&price="+element.value;
				var url = "../repository_queries/admin_affiliate_updateproductpricing.php?"+query;
				params = "";
				ajaxQuery(url, params, 'nothing');
			}
