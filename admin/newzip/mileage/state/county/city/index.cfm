...
<CFSWITCH expression="#url.action#">
    ...
    <CFQUERY name="qZips" datasource="#request.db.dsn#">
           select b.zipid as newzip
			from zip_code b
            where state_prefix='#form.state#' and county='#form.county#' and city='#form.city#'
		...
		<CFLOOP list="#lFields#" index="i">
			<CFIF FindNoCase('Tour_',i) neq 0>
				...
				<CFQUERY name="qPrevious" datasource="#request.dsn#">
                    ...
                    and city_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.cityid,2,'_')#" />
                    and zip_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#qZips.newzip#" />
				...
				<CFIF Trim(Evaluate('form.' & i)) neq "">
					...
					<CFELSE>
						<CFQUERY name="qCount" datasource="#request.dsn#">
							select max(pb_id) as mynum from pricing_zips_mileage 
						</CFQUERY>
						<CFQUERY name="qInsert" datasource="#request.dsn#">
							insert into pricing_zips_mileage (pb_id, state_id,county_id,city_id,zip_id, tourtype_id, unitprice,hide)
							values (
                                ...
                                <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.cityID,2,'_')#" />,
                                <cfqueryparam cfsqltype="cf_sql_integer" value="#qZips.newzip#" />,
				...
				<CFELSE>
					...
					<CFIF qPrevious.RecordCount gt 0>
						<CFQUERY name="qDelete" datasource="#request.dsn#">
							delete from pricing_zips_mileage
							where state_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.stateID,2,'_')#" />
							and county_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.countyID,2,'_')#" />
                            and city_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.cityid,2,'_')#" />
                            and zip_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#qZips.newzip#" />
			...
			<CFIF FindNoCase('Tour_',i) neq 0>
				...
				<CFIF Trim(Evaluate('form.' & i)) neq "">
					...
					<CFELSE>
						...
						<CFQUERY name="qInsert" datasource="#request.dsn#">
							insert into pricing_cities_mileage (pb_id, state_id,county_id,city_id, tourtype_id, unitprice,hide)
							values (
								<cfqueryparam cfsqltype="cf_sql_integer" value="#Evaluate(qCount.mynum + 1)#" />,
								<cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.stateID,2,'_')#" />,
                                <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.countyID,2,'_')#" />,
				...
				<CFELSE>
					...
					<CFIF qPrevious.RecordCount gt 0>
						<CFQUERY name="qDelete" datasource="#request.dsn#">
							delete from pricing_cities_mileage
							where state_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.stateID,2,'_')#" />
							and county_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.countyID,2,'_')#" />
                            and city_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.cityid,2,'_')#" />
		...
		<CFSET url.cityid = ListGetAt(form.cityid,2,'_') />
        <CFSET url.state = form.state />
        <CFSET url.city = form.city />
        <CFSET url.county = form.county />
	</CFCASE>
    <CFCASE value="updatemileagePrice">
		<CFSET lFields = form.fieldnames />
		<CFLOOP list="#lFields#" index="i">
			<CFIF FindNoCase('Tour_',i) neq 0>
				...
				<CFIF Trim(Evaluate('form.' & i)) neq "">
					<CFIF qPrevious.RecordCount gt 0>
						<CFQUERY name="qUpdate" datasource="#request.dsn#">
							update pricing_brokers_mileage
							set unitprice = <cfqueryparam cfsqltype="cf_sql_decimal" value="#Evaluate('form.' & i)#">,
                            	hide=<cfif isdefined('form.hide'&ListGetAt(i,2,'_'))>'1'<cfelse>'0'</cfif>,
					...
					<CFELSE>
						<CFQUERY name="qCount" datasource="#request.dsn#">
							select max(pb_id) as mynum from pricing_brokers_mileage 
						</CFQUERY>
						<CFQUERY name="qInsert" datasource="#request.dsn#">
							insert into pricing_brokers_mileage (pb_id, brokerage_id, product_id, unitprice,hide,broker_billable)
							values (
								<cfqueryparam cfsqltype="cf_sql_integer" value="#Evaluate(qCount.mynum + 1)#" />,
				...
				<CFELSE>
					...
					<CFIF qPrevious.RecordCount gt 0>
						<CFQUERY name="qDelete" datasource="#request.dsn#">
							delete from pricing_brokers_mileage
							where brokerage_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(form.brokerage,2,'_')#" />
							and product_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#ListGetAt(i,2,'_')#" />
						</CFQUERY>
					</CFIF>
...
<CFELSEIF url.pg eq "editmileagepricing">
	<CFINCLUDE template="_editmileagepricing.cfm">
