<?PHP
	...
	if(isset($_REQUEST['memberID'])){
		...
		if($member->loadMemberInfoByID($memberID)){
			...
			$userName = $users->getName($memberID);
			$brokerID = $users->getBrokerID($userID);
			$broker =  new brokerages();
			$broker->loadBrokerage($brokerID);
		}
	}

	$brokerageUsersCount = $broker->countUsers($brokerID);
	$brokerageUsers = $broker->getUsers($brokerID);
	$userInfo = [
		'memberID = ' . $memberID,
		'userID = ' . $userID,
		'brokerID = ' . $brokerID,
		'broker usr cnt = ' . $brokerageUsersCount
	];
?>
...
<body>
<div class="container">
	<h2 align="center">My Brokerage Listings</h2>
	<h3 align="center"><strong>To-Do:</strong> "Coming soon" sections, Get user office ID rather than hard coded, create section to choose what to automatic or proof email for these listings, deal with user selected option, add price range and zip code filter for what to show.</h3>
...
<script src='includes/select-brokerage-listing-dev.js'></script>
<script>
	memberID = <?php echo $memberID ?>;
	userID = <?php echo $userID ?>;
	brokerID = <?php echo $brokerID; ?>;
	getListings();
