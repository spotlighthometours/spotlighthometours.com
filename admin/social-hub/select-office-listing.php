<?PHP
	...
	if(isset($_REQUEST['memberID'])){
		...
		if($member->loadMemberInfoByID($memberID)){
			...
			// $userMLSID = $users->getMlsID($userID);
			$broker =  new brokerages();
			$broker->loadBrokerage($brokerID);
			$userMLSProvider = $users->mls[0]['mlsProvider'];
			$userMLSID = $users->mls[0]['mlsID'];
			$mls = new mls();
			$mlsProvider = $mls->providerFactory($userMLSProvider);
			$brokerIDs = $mls->getBrokerageIDs($brokerID);
			$officeID = $brokerIDs[0]['mlsID'];
		}
	}
	...
	$viewInfo = [
		'memberID = ' . $memberID,
		'userID = ' . $userID,
		'user\'s mlsProvider = ' . $userMLSProvider,
		'brokerID = ' . $brokerID,
		'mlsProvider = ' . $userMLSProvider,
		'brokerageOffice = ' . $officeID
	];
?>
...
<script src='includes/select-office-listing.js'></script>
<script>
	memberID = <?php echo $memberID ?>;
	userID = <?php echo $userID ?>;
	brokerID = <?php echo $brokerID; ?>;
	officeID = <?php echo $officeID; ?>;
	getListings();
