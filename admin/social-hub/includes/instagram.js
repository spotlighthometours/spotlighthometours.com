...
$(".instagram-open").on("click", function(){
	postToInstagram();
});

function postToInstagram(){
	var mediaURL = $(".post .feed-img").attr('src');
	window.location = '../../repository_queries/streamdownload.php?url='+mediaURL;
	setInterval(checkDownloadComplete,1000);
}

function checkDownloadComplete(){
	$.ajax({
	  url: "../../repository_queries/isdownloadcomplete.php",
	  cache: false
