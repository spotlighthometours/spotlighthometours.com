...
<!---<cfdump var="#qGetBrokerDetails#" />--->
    ...
    <CFQUERY name="qGetTourDetails" datasource="#request.db.dsn#">
        ...
        (select fullname from photographers where photographerID=tp.mediaphotographer limit 1) as mediarephotographer
    ...
    <CFQUERY name="qTours" datasource="#request.db.dsn#">
        select t.tourid, t.tourTypeID,(SELECT group_concat(mlsID separator ', ') FROM tour_to_mls WHERE tourID=t.tourID) as mls,t.address,t.title,date_format(createdOn,'%m/%d/%Y - %h:%i:%s %p') as dateord,(select tourTypeName from tourTypes where tourTypeID = t.tourTypeID) as tourTypeN from tours t
...
<CFFUNCTION name="sendEmailNotification" access="public" hint="Send Email Notification To User">
    ...
    <CFQUERY name="qTours" datasource="#request.db.dsn#">
        select t.tourid,(SELECT group_concat(mlsID separator ', ') FROM tour_to_mls WHERE tourID=t.tourID) as mls,t.address,t.title,t.tourTypeID,date_format(createdOn,'%m/%d/%Y - %h:%i:%s %p') as dateord,(select tourTypeName from tourTypes where tourTypeID = t.tourTypeID) as tourTypeN from tours t
        where t.tourID =<cfqueryparam cfsqltype="cf_sql_int" value="#arguments.tourid#">
    </CFQUERY>
...
/>
<cfset tourMsg = true>
<cfif qTours.tourTypeID eq 48>
	<cfset tourMsg = true>
</cfif>
<cfif qTours.tourTypeID eq 39>
	<cfset tourMsg = true>
</cfif>
<cfif tourMsg>
	<cfmail port="465" useSSL="true" useTLS="true" attributeCollection="#mailAttributes#">
...
Great news! Your tour for #qTours.address#. is online and available for viewing by clicking the link below: 

http://www.spotlighthometours.com/tours/tour.cfm?tourid=#qTours.tourid# 

You can also download the high and low resolution images by logging into 
your account with the link below. 

http://www.spotlighthometours.com/users/login.cfm 

We will attempt to add the tour to the MLS for you. However if there is already photos or a previous tour under the property tour section the MLS system will not let us override it.

If this is the case please contact your office administrator to change the tour provider to Spotlighthometours. The links provided below can be used for your local MLS for the branded and non branded links. Just add the MLS number to the end of the links below. 

Non contact or unbranded for MLS 

http://www.spotlighthometours.com/tours/tour.cfm?mls=#qTours.mls# 

contact or branded  public 

http://www.spotlighthometours.com/tours/tour.cfm?contact=1&mls=#qTours.mls# 

If you did not have a MLS number when the tour was ordered please give us a call or contact your office administrator to add it.  

Please feel free to contact us with any questions or problems with your 
tour. 
...
<CFIF  REQUEST.RequestCommitted eq false >
    ...
    <CFSET temp1= sendTextNotification(arguments.notifyType,arguments.notifyString,arguments.userid,strTo,arguments.tourid) />
        ...
        <CFIF qSendUserNotification.email neq "">
	           <!--- <cfset temp2=sendEmailNotification(arguments.notifyType,arguments.notifyString,arguments.userid,strToe,arguments.tourid) />
    --->
...
<CFIF isDefined('form.save') OR isDefined('form.savenotify')>
	<CFSET url.tour = form.tour />
    <CFSET url.user = form.user />
	<CFSET notify = 0 />
    
    <CFIF isDefined('form.savenotify')>
    	<CFSET notify = 1 />
    </CFIF>
...
<CFIF form.stage eq 1 and form.ScheduleAttemptedon eq "">
	<CFSET errorMsg="Please select the date and time for Schedule Attempt">
</CFIF>
<CFIF form.stage eq 2 and form.Scheduledon eq "" and form.check eq 1>
	<CFSET errorMsg="Please select the date and time for Scheduled">
</CFIF>
<CFIF form.stage eq 3 and form.MediaReceivedon eq "">
	<CFSET errorMsg="Please select the date and time for Media Received">
</CFIF>
<CFIF form.stage eq 4 and form.Editedon eq "">
	<CFSET errorMsg="Please select the date and time for Media Edited">
</CFIF>
...
<CFIF not len(errorMsg)>
   ...
   photographer='#form.photographer#',
	...
	<cfelse>
		Scheduledon = <cfqueryparam value="#form.Scheduledon#" null="true">,
    </cfif>
	<cfif form.stage eq 1>
        ScheduleAttempted='#form.check#',
    ...
    <cfif isDefined('form.TourBuilt')>
        TourBuilt='1',
        TourBuilton=now(),
    <cfelse>
        TourBuilt='0',        
    </cfif>
    <cfif isDefined('form.Realtorcom')>
        Realtorcom='1',
        Realtorcomon=now(),
    <cfelse>
         Realtorcom='0',
    </cfif>
    <cfif isDefined('form.mls') OR form.stage eq 8>
        mls='1',
        mlson=now(),
    <cfelse>
         mls='0',
    ...
    <cfif form.stage eq 8>
        finalized='#form.check#',
    ...
    <CFIF form.stage eq 1 AND isDefined('form.savenotify')>
    <CFOUTPUT>scheduleattempt<br></CFOUTPUT>
    	<CFSET tempk0=sendNotification('scheduledatt','scheduleattempt',form.tour,form.user) />
    </CFIF>
    
     <CFIF form.stage eq 2 AND isDefined('form.savenotify')>
    	<CFSET tempk1= sendNotification('scheduled','scheduled',form.tour,form.user) />
    </CFIF>
    
    <CFIF form.stage eq 8 AND isDefined('form.savenotify')>
    ...
    <CFCASE value="insertUser">
		...
		<CFELSE>
			...
			<CFQUERY datasource="#request.db.dsn#">
				insert into users (firstName, lastName, userType, salesRepID, BrokerageID, otherBrokerage, username, password, address, city, state, zipCode, phone, phonecarrier,phone2, fax, email, assistName, assistPhone, agentNotes, uri, uploadPrivilege, dateCreated, dateModified,sms,emailUnsuscribe)
				values (<cfqueryparam value="#trim(form.firstName)#" cfsqltype="cf_sql_varchar" maxlength="20">,
						...
						<cfqueryparam value="#trim(form.brokerageID)#" cfsqltype="cf_sql_integer" maxlength="9">,
						...
						#now()#,
						#now()#,
					<cfif qBrokeragePlans.RecordCount gt 0 OR StructKeyExists(form,"previewEnabled")>
						1,
					<cfelse>
						0,
					</cfif>
	...
	<CFCASE value="updateUser">
		...
		<CFQUERY name="qOldBrokerage" datasource="#request.dsn#">
			select brokerageID, sms from users where userid = <cfqueryparam cfsqltype="cf_sql_integer" value="#form.userID#" />
		</CFQUERY>

		<CFSET strSMSTemp = qOldBrokerage.sms />
		...
		<!--- No matter what, the preview enabled admin setting over-rides a new brokerage setting --->
		<CFIF StructKeyExists(form,"previewEnabled")>
			<CFSET strSMSTemp = 1 />
		<CFELSE>
			<CFSET strSMSTemp = 0 />
		</CFIF>
		...
		<CFQUERY datasource="#request.db.dsn#">
			update users set
                ...
                expressUser = <cfif isDefined('form.expressUser')>1<cfelse>0</cfif>,
				socialHub = <cfif isDefined('form.socialHub')>1<cfelse>0</cfif>,
				dateModified = #now()#,
				sms = #strSMSTemp#,
		...
		<!-- CHECK THE DIY MEMBER STATUS FOR THIS USER -->
		<cfif isDefined('form.expressUser')>
			<CFSET expressUser = 1>
		<cfelse>
			<CFSET expressUser = 0>
		</cfif>
		
		<CFQUERY name="qDIYMember" datasource="#request.dsn#">
			SELECT active FROM members WHERE typeID='1' AND userID = <cfqueryparam cfsqltype="cf_sql_integer" value="#form.userID#" /> LIMIT 1
		</CFQUERY>
		
		<cfif qDIYMember.recordcount is 0>
			<CFSET diyActive = 0>
			<CFSET diyInserted = false>
		<cfelse>
			<CFSET diyActive = qDIYMember.active>
			<CFSET diyInserted = true>
		</cfif>
			
		<cfif expressUser eq 1>
			<cfif diyActive eq 0>
				<cfif diyInserted>
					<CFQUERY datasource="#request.db.dsn#">
						UPDATE members SET
						active = <cfqueryparam value="1" cfsqltype="cf_sql_int">
						WHERE userID = #form.userID# AND typeID='1'
					</CFQUERY>
				<cfelse>
					<CFQUERY datasource="#request.db.dsn#">
						INSERT INTO members (typeID, userID, trial, createDate, active)
						VALUES (1, <cfqueryparam value="#form.userID#" cfsqltype="cf_sql_int">, 0, now(), <cfqueryparam value="1" cfsqltype="cf_sql_int">)
					</CFQUERY>
				</cfif>
			</cfif>
		<cfelse>
			<cfif diyInserted>
				<CFQUERY datasource="#request.db.dsn#">
					UPDATE members SET
					active = <cfqueryparam value="0" cfsqltype="cf_sql_int">
					WHERE typeID='1' 
