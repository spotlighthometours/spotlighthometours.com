<CFSILENT>
    ...
    <CFPARAM name="url.cspusers" default="" />
    ...
    <CFQUERY name="qUsers" datasource="#request.db.dsn#">
        ...
        <cfif len(url.username) gt 0>
        	WHERE u.username LIKE '%#url.username#%'
        </cfif>
        <cfif len(url.BrokerageID) gt 0>
        ...
        <cfif len(url.cspusers) gt 0>
            <cfif len(url.name) gt 0 OR len(url.userid) gt 0 OR len(url.BrokerageID) gt 0>
                AND
            <cfelse>
                WHERE
            </cfif> 
            (SELECT COUNT(id) FROM members WHERE typeID='23' AND active='1' AND userID=u.userID)>0 
        </cfif>
        ORDER BY #url.orderby# LIMIT #pIndex#,#url.itemsperpage#
    </CFQUERY>
...
<TABLE WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="0">
  ...
  <TR>
    <TD WIDTH="10%">Filter by Username:</TD>
    <TD WIDTH="90%"><INPUT NAME="username" TYPE="text" value="#url.username#"></TD>
  </TR>
  <TR>
    ...
    <TD><SELECT NAME="BrokerageID">
            ...
            <CFLOOP query="qBrokerages">
              <OPTION VALUE="#BrokerageID#"<cfif url.BrokerageID eq BrokerageID> selected</cfif>>#BrokerageName# (desc:#brokerageDesc#) ID: #BrokerageID#</OPTION>
            </CFLOOP>
          </SELECT></TD>
  </TR>
  <TR>
...
<TABLE WIDTH="95%" BORDER="0" CELLSPACING="2" CELLPADDING="2">
  ...
  <TH><A HREF="#cgi.script_name#?cspusers=true&name=#url.name#&brokerageid=#brokerageID#">CSP Content</a></TH>
  ...
  <CFLOOP query="qUsers">
   ...
   <cfquery name="qUserConciergeSocPlat" datasource="#request.db.dsn#">
		SELECT id FROM members WHERE typeID='23' AND active='1' AND userID='#userID#'
	</cfquery>
  ...
  <TR BGCOLOR="###iif(currentRow mod 2, de("E8EEF7"), de("ffffff"))#">
    ...
    <TD><cfif qUserConciergeSocPlat.recordcount gt 0><a href="http://www.spotlighthometours.com/admin/social-hub/?userType=user&userID=#userID#" target="_blank">CS Plat CM</a></cfif></TD>
