...
<cfif editMode>
    ...
    <cfquery name="qAffiliatePhotographers" datasource="#request.db.dsn#">
    	SELECT * FROM photographers WHERE isAffiliate = 1 AND active = 1
    </cfquery>
...
<form action="#cgi.sript_name#?action=<cfif editMode>updateUser<cfelse>insertUser</cfif>" method="post">
    <table width="500" border="0" cellspacing="2" cellpadding="4">
      ...
      <tr>
        <td class="rowHead">MLS<br>
        (Agent ID)</td>
          <td class="rowData"><input name="mls" type="text" size="20" maxlength="20"<cfif editMode> value="#qUsers.mls#"</cfif>>
        	MLS Provider: <select name="mlsProviderID">
            <option value=""<cfif not editMode or not qUsers.mlsProviderID gt 0> selected</cfif>>Select One...</option>
      ...
      <TR>
        <TD CLASS="rowHead">Affiliate Photographer</TD>
        <TD CLASS="rowData">
		  <SELECT NAME="affiliatePhotographerID">
          	<option value="0"<cfif not editMode or #qUsers.affiliatePhotographerID# eq 0> selected</cfif> >-</option>
            <CFLOOP query="qAffiliatePhotographers">
		  	   	<option value="#qAffiliatePhotographers.PhotographerID#" <cfif editMode and #qUsers.affiliatePhotographerID# eq #qAffiliatePhotographers.PhotographerID#> selected</cfif> >#qAffiliatePhotographers.fullName#</option>
            </CFLOOP>    
		  </SELECT>
		</TD>
	  </TR>
