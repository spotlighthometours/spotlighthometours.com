...
Photographer: <cfif #qGetTourDetails.ReScheduledDiff# gt 0>#qGetTourDetails.ReScheduledPhotographer#<cfelse>#qGetTourDetails.ScheduledPhotographer#</cfif>
                    ...
                    <CFIF #qTours.tourCategory# eq 'Video Tours' && (#qGetTourDetails.VideoScheduledDiff# gt 0 || #qGetTourDetails.VideoReScheduledDiff# gt 0)>
...
<CFCASE value="updateProgress,cancelPhotoProgress,cancelVideoProgress">
    ...
    <CFSET videoTour = false>
    <CFLOOP query="qToursAdditional">
        <cfif FindNoCase("Cinematic", qToursAdditional.productname) gt 0>
            <CFSET videoTour = true>
        </cfif>
    </CFLOOP>
    <cfif videoTour eq true>
        <CFSET videoTour = true>
    </cfif>
    ...
    <CFIF form.stage eq 2>
        <!---<cfif form.Scheduledon eq "" && videoTour eq false>
        ...
        <!---<cfif videoTour eq true && form.VideoScheduledOn eq "" && form.Scheduledon eq "">
            <CFSET errorMsg="Please select the date and time for Photo/Video Scheduled Date/Time">
        </cfif> --->
        <cfif videoTour eq true && form.VideoScheduledOn neq "" && form.VideoPhotographer eq "0">
        ...
        <cfif videoTour eq true && form.VideoReScheduledOn neq "" && form.VideoRePhotographer eq "0">
    ...
    <CFIF not len(errorMsg)>
        ...
        <CFQUERY name="qTourProgressUpdate" datasource="#request.db.dsn#">
                UPDATE tourprogress set 
                    ...
                    <cfif val(form.finalized) eq 1 <!---&& qTours.finalized eq 0--->>
                    ...
                    isVideoTour = <cfif videoTour eq true>1<cfelse>0</cfif>
        ...
        <CFIF #notify#>
            ...
            <CFELSEIF #form.stage# gt 2 && val(form.VideoFinalized) && qTours.VideoFinalized && val(form.finalized) && qTours.finalized>
                <CFOUTPUT>Tour Updated<br></CFOUTPUT>
                <CFSET  tempk7= sendNotification('finalized','tour updated',form.tour,form.user) />
                <CFOUTPUT>Tour Updated Teams<br></CFOUTPUT>
                <CFSET  tempk7= sendNotificationTeams('finalized','tour updated',form.tour,form.user) />
            <CFELSEIF val(form.VideoFinalized)>
                <CFOUTPUT>Video Finalized<br></CFOUTPUT>
                <CFSET  tempk3= sendNotification('finalized','video finalized',form.tour,form.user) />
                <CFOUTPUT>Video Finalized Teams<br></CFOUTPUT>
                <CFSET  tempk7= sendNotificationTeams('finalized','video finalized',form.tour,form.user) />
            <CFELSE>
            	<CFIF val(form.finalized)>
...
<CFCASE value="insertUser">
		...
		<CFELSE>
            ...
            <CFQUERY datasource="#request.db.dsn#">
				insert into users (firstName, lastName, userType, salesRepID, BrokerageID, otherBrokerage, username, password, address, city, state, zipCode, phone, phonecarrier,phone2, fax, email, assistName, assistPhone, agentNotes, salesNotes, uri, uploadPrivilege, dateCreated, dateModified,emailUnsuscribe,mlsProviderID,TourWindowType,affiliatePhotographerID)
				values (<cfqueryparam value="#trim(form.firstName)#" cfsqltype="cf_sql_varchar" maxlength="20">,
						...
						<cfqueryparam value="#trim(form.affiliatePhotographerID)#" cfsqltype="cf_sql_integer" maxlength="9">
						)
			</CFQUERY>
	...
	<CFCASE value="updateUser">
		...
		<CFQUERY datasource="#request.db.dsn#">
			update users set
                ...
                affiliatePhotographerID = <cfqueryparam value="#trim(form.affiliatePhotographerID)#" cfsqltype="cf_sql_integer" maxlength="9"> 
