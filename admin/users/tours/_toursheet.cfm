...
<CFIF qUser.salesRepID neq 0 AND Val( qUser.salesRepId ) EQ qUser.salesRepId>
...
<CFQUERY name="isAgentWebsite" datasource="#request.db.dsn#">
    SELECT COUNT(m.id) as isagent FROM members m, tours t, users u WHERE t.tourID = <cfqueryparam value="#url.tour#" cfsqltype="cf_sql_integer" /> AND t.userID = u.userID AND ((m.userType = 'user' AND m.userID = u.userID) OR (m.userType = 'broker' AND m.userID = u.brokerageID)) AND m.active = 1 AND m.typeID=17
</CFQUERY>

<CFSET isagentweb = false />  
...
<CFCONTENT reset="no">
<cfloop query="isAgentWebsite">
	 <cfif #isagent# gt 0>
     	<CFSET isagentweb = true />
     </cfif>
</cfloop>
<HTML>
<HEAD>
...
<LINK HREF="/repository_css/template.css" REL="stylesheet" TYPE="text/css">
...
<style>
	...
	#backdrop {
		...
		height: 100%;
		width: 100%;
		opacity: 0.7; /* Firefox, Safari and Chrome (WebKit), Opera */
	...
	/* <![CDATA[ */
		$(function() {
			...
			$("#save").click(function(event){
				...
				$(this).css({color: 'rgb(217,217,217)'});
				var that = this;
				setTimeout(function(){ $(that).attr("disabled", false); }, 3000);
				validate(event);
			});
            $("#header td").css("background-color",$("#tableHeader").attr("bgcolor"));
			...
			$.ajax({
				url: '/admin/users/tours/upgrade-history.php?tourId='+ $("input[name='tour']").val(),
				type: 'GET',
			}).done(function(msg){
				if( msg.length == 0 ){
					$("#upgradeHistoryLink").html("");
				}
			});
		});
		var today = new Date();
		...
		function upgrade(){
			$.ajax({
				type: "GET",
				url: "/admin/users/tours/upgrade.php?tourId=" + $("input[name='tour']").val(),
			}).done(function(msg){
				//
				ShowPopUp("Upgrade tour",msg);
				$("#upgradeOptions").bind("change",function(){
					if($(this).val() != 'z' ){
						$("#upgradeSaveWrapper").slideDown();
					}else{
						$("#upgradeSaveWrapper").slideUp();
					}
				});
				
				$("#upgradeSaveButton").bind("click",function(){
					$.ajax({
						type: "POST",
						url: "/admin/users/tours/upgrade.php?ajax=1&mode=save&tourId=" + $("input[name='tour']").val(), 
						data:{
							'from': $("#upgradeTourTypeId").val(),
							'to': $("#upgradeOptions").val()
						}
					}).done(function(msg){
						$("#upgrade-wrapper").html(
							"<img src='/admin/users/tours/great-job.jpg'>"
						);
					});
		...
		function cancelSchedule(video){
			$.ajax({
				type: "GET",
				url: "/admin/users/tours/toursheet-cancel.php?tourId=" + $("input[name='tour']").val(),
			}).done(function(msg){
				ShowPopUp("Cancel Shoot",msg);
				$('#rescheduleDate').datetimepicker({
					showSecond: true,
					dateFormat: 'yy-mm-dd',
					timeFormat: 'hh:mm:ss'
				});
				$("#reschedule").bind("click",function(){
					if($(this).is(":checked")){
						$("#rescheduleContainer").slideDown();
					}else{
						$("#rescheduleContainer").slideUp();
					}
				});
				$("#cancelScheduleButton").bind("click",function(){
					;
					if( $("#reschedule").is(":checked") ){
						if( $("#rescheduleDate").val().length == 0 ){
							return alert("Please select a reschedule date");
						}
						if( $("#photoglist").val() == 0 ){
							return alert("Please select a photographer");
						}
						if( video ){
							$("#VideoReScheduledOn").val( $("#rescheduleDate").val() );
							$("#VideoRePhotographer").val( $("#photoglist").val() );
						}else{
							$("#ReScheduledon").val( $("#rescheduleDate").val() );
							$("select[name='rephotographer']").val( $("#photoglist").val() );
						}
					}
					checked = $("input[type='radio'][name='billed']:checked").prop("id");
					if( checked.length > 0 ){
						if( checked == "agentBilled" ){
							if( video ){
								$("#VideoScheduledNotes").val( $("#VideoScheduledNotes").val() + "\nAgent billed[" + todayDate + "]" );
							}else{
								$("#ScheduledNotes").val($("#ScheduledNotes").val() +"\nAgent Billed["+todayDate +"]" );
							}
						}else if( checked == "agentNeedsBilling" ){
							if( video ){
								$("#VideoScheduledNotes").val( $("#VideoScheduledNotes").val() + "\nAgent needs billing[" + todayDate + "]" );
							}else{
								$("#ScheduledNotes").val($("#ScheduledNotes").val()+"\nAgent needs billing["+todayDate+"]");
							}
						}else if( checked == "agentNotBilled" ){
							if( video ){
								$("#VideoScheduledNotes").val( $("#VideoScheduledNotes").val() + "\nAgent not billed[" + todayDate + "]" );
							}else{
								$("#ScheduledNotes").val($("#ScheduledNotes").val()+"\nAgent not billed["+todayDate+"]");
							}
						}
					}
					if( $("#canceledBy").val().length && $("#canceledBy").val() != 'who dat is'){
						if( video ){
							$("#VideoScheduledNotes").val( $("#VideoScheduledNotes").val() + "\n**Cancelled by:" + $("#canceledBy").val() );
						}else{
							$("#ScheduledNotes").val($("#ScheduledNotes").val()+"\n**Cancelled by:" + $("#canceledBy").val());
						}
					}
					...
					$.ajax({
						url: "/repository_queries/admin_notify_cancellation.php",
						type: "POST",
						data:{
							"video": (video ? 1 : 0),
							"tourId": $("input[name='tour']").val()
						}
					}).done(function(msg){
						_cancelSchedule(video);
					});
				});
			});
		}
		
		function _cancelSchedule(video){
			var startIt = "";
            tourId = $("input[name='tour']").val();
            var d;
			if (video) {
			...
			$.ajax({
                type: 'POST',
                url: '/repository_queries/user_gc_schedule.php',
                data: d
            }).done(function(msg){
        ...
        function SaveSubmit(doSubmit,cancelType,tourId){
				...
				$('#savenotify').css({color: 'rgb(217,217,217)'});

            tourId = $("input[name='tour']").val();
            scheduledOn = $("#Scheduledon").val();
            photog = $("#photographer option:selected").val();
            rescheduleOn = $("#ReScheduledon").val();
            rephotog = $("select[name='rephotographer'] option:selected").val();
            vidScheduledOn = $("#VideoScheduledOn").val();
            vidPhotog = $("#VideoPhotographer option:selected").val();
            vidRescheduledOn = $("#VideoReScheduledOn").val();
            vidRePhotog = $("select[name='VideoRePhotographer'] option:selected").val();
            $.ajax({
                type: 'POST',
                url: '/repository_queries/user_gc_schedule.php',
                data: {
                    ...
                    'rephotog' : rephotog,
                    'vidScheduledOn' : vidScheduledOn,
                    'vidPhotog' : vidPhotog,
                    'vidRescheduledOn': vidRescheduledOn,
                    'vidRePhotog' : vidRePhotog
                }
            }).done(function(msg){
                //
                _SaveSubmit(doSubmit,cancelType,tourId);
            });
        }
        
		function _SaveSubmit(doSubmit, cancelType, tourId ) {
...
<TABLE WIDTH="840" BORDER="0" CELLSPACING="0" CELLPADDING="0">
  ...
  <TR>
    <TD><TABLE id='tableHeader' WIDTH="100%" BORDER="0" CELLPADDING="0" CELLSPACING="0" BGCOLOR="#color#">
      ...
      <TR STYLE="color:##FFFFFF;font-size:10px;" ID="header">
        ...
        <TD align="center" valign="bottom">
          Reshoot<BR>
          <INPUT NAME="ReshootFinalized" TYPE="checkbox" ID="VideoFinalized" VALUE=1 <cfif qTourPro.ReshootFinalized>checked</cfif>>
        </TD>
        <TD align="center" valign="bottom">
      ...
      <TR>
        <TD WIDTH="40%"><TABLE WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="0">
          <TR>
            ...
            <TD WIDTH="62%">
				<SPAN CLASS="style1">
					<a href="javascript:void(0)" onClick="window.open('../tour-type-details.php?tourTypeID=#qTours.tourTypeID#','Tour Type Details','height=400,width=600')">
						#qTours.tourTypeN#
					</a>::
					<a href='javascript:void(0)' id='upgradeLink' onClick='upgrade()'>Upgrade</a> 
					<a href='javascript:void(0)' id='upgradeHistoryLink' onClick='upgradeHistory()'>| Upgrade History</a>
				</SPAN>
            </TD>
          </TR>
          <TR>
    ...
    <TD HEIGHT="69"><TABLE WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="0">
      ...
      <TR>
        ...
        <CFLOOP query="qToursAdditional">
			...
			<CFIF productid eq 137>
                <CFSET microsite = 1>
            </CFIF>
            <CFIF productid eq 55>
                <CFSET propertyUrl = 1>
            </CFIF>
        ...
        <CFIF microsite eq 1>
            <script src='url-toursheet.js?rand=#Rand("SHA1PRNG")#'></script>
            <script>
                drawMicrositeUrlList($("input[name='tour']").val(),"##urlList");
            </script>
        </CFIF>
        <CFIF propertyUrl eq 1>
            <script src='url-toursheet.js?rand=#Rand("SHA1PRNG")#'></script>
            <script>
                drawPropertyUrlList($("input[name='tour']").val(),"##urlList");
            </script>
        </CFIF>
            <div id='urlList'></div>
        </TD>
  ...
  <TR>
    <TD><BR>
	<CFIF isagentweb><h3 align="center" style="padding:10px;border:1px solid green;">THE USER WHO OWNS THIS TOUR HAS A REALTOR WEBSITE, PLEASE CREATE A MY LISTINGS VIDEO.</h3></CFIF>
    <TABLE WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="0">
      <TR>
