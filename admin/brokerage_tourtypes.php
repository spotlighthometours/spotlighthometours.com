...
$blackListedTourTypes = $brokerage->getBlacklistedTourTypes();
$regionalAllowedTourTypes = $brokerage->getRegionalAllowedTourTypes();
...
$tourTypePricing = $brokerage->getTourTypePricing();
...
<script>
	function updateRegionalSelection(id){
		if($('#regional_'+id).is(':checked')){
			addSelection(id, "1");
		}else{
			removeSelection(id, "1");
		}
	}
	...
	function addSelection(id, regional){
		query = "action=add&id="+<?PHP echo $_GET['id']; ?>+"&tourTypeID="+id+"&regional="+regional;
		var url = "../repository_queries/admin_brokerage_updatetourtypes.php?"+query;
		params = "";
		//alert("add"+query);
	...
	function removeSelection(id, regional){
		query = "action=remove&id="+<?PHP echo $_GET['id']; ?>+"&tourTypeID="+id+"&regional="+regional;
		var url = "../repository_queries/admin_brokerage_updatetourtypes.php?"+query;
		params = "";
		//alert("remove"+query);
		ajaxQuery(url, params, 'nothing');
	...
	function updatePrice(element){
		...
		if(element.value=="" && queryPost.length > 0){
			query = "action=remove&id="+<?PHP echo $_GET['id']; ?>+"&tourTypeID="+id+queryPost;
			var url = "../repository_queries/admin_brokerage_updatetourtypepricing.php?"+query;
			params = "";
			//alert (url);
			ajaxQuery(url, params, 'nothing');
		}else{
			...
			if(value.search(strPattern)== -1){
				...
				query = "action=add&id="+<?PHP echo $_GET['id']; ?>+"&tourTypeID="+id+queryPost+"&price="+value;
	...
	function deselectAll(){
		var proceed = confirm("Are you sure you want to remove all tour types from this brokerage?");
		if(proceed){
			...
			alert('Removing all Tour Types from this brokerage!');
		}
	}
	
	function selectAll(){
		var proceed = confirm("Are you sure you want to add all tour types to this brokerage?");
		if(proceed){
			$('input[type=checkbox]').each(function () {
				if($(this).attr('id').indexOf("tourType") >= 0){
					$(this).parent().show();
			...
			alert('Adding all Tour Types to this brokerage!');
		}
	}
</script>
<style>
...
<?PHP
echo ($affiliatePhotographerID>0)?'<h4>[ <span style=\'cursor:pointer\' onclick="window.location=\'http://www.spotlighthometours.com/admin/affiliate_tourtypes.php?id='.$affiliatePhotographerID.'\'">Load affiliate\'s available tour types</span> ]</h4>':'';
?>
...
?>
    ...
    <?PHP echo $column['tourTypeName'] ?> 
    <input id="regional_<?PHP echo $column['tourTypeID']; ?>" type="checkbox" <?PHP echo (in_array($column['tourTypeID'], $regionalAllowedTourTypes))?'CHECKED':'' ?> onchange="updateRegionalSelection(<?PHP echo $column['tourTypeID']; ?>)"/>Allow Regional 
    <br/>
    <?PHP
