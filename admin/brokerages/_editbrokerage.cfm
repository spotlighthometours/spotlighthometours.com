...
<form enctype="multipart/form-data" action="#cgi.sript_name#?action=<cfif editMode>updateBrokerage<cfelse>insertBrokerage</cfif>" method="post">
    <table width="500" border="0" cellspacing="2" cellpadding="4">
      ...
      <tr>
        ...
        <td class="rowData"><input type="file" id='logo_file' name="logo_file">
        		<input type="hidden" id="delete_logo" name="delete_logo" value="0" />
                <cfif isdefined("qBrokerages.logo") >
                <input type="hidden" name="old_logo" value="#qBrokerages.logo#" />
                </cfif>
				<cfif editMode and fileExists(expandPath('/images/logos/#qBrokerages.logo#'))>
				<br><img id="logo_img" src="/images/logos/#qBrokerages.logo#">
                <br><a id="delete_logo_link" onClick="javascript: document.getElementById('delete_logo').value='1'; document.getElementById('logo_img').style.display='none'; this.style.display='none'; document.getElementById('del_msg').style.display='block';" style="cursor:pointer">[ DELETE LOGO ]</a>
                <div id="del_msg" style="display:none; font-weight:bold;">Click "Update Brokerage" To Delete Logo <br> <a  onclick="javascript: document.getElementById('delete_logo').value='0'; document.getElementById('logo_img').style.display='block'; document.getElementById('del_msg').style.display='none'; document.getElementById('delete_logo_link').style.display='block';" style="cursor:pointer">[ RESTORE LOGO ]</a></div>
				</cfif>				</td>
      </tr>
			<tr> 
