 ...
 * Date: 09-28-2015
 * Purpose: Display email campaign leads
 */
    ...
    if( isset($_POST['draw']) ){
        ...
        $query = "SELECT SQL_CALC_FOUND_ROWS ec.title,ecc.campaignId, ecc.dateCreated, u.userID, CONCAT(u.lastName,', ',u.firstName) as firstName, ";
        $query .= " u.username, u.brokerageID, brokerageName, otherBrokerage, brokerageDesc,  u.city,u.state ";
        /*
		...
		*/
        $query .= " FROM emailcampaignclicks ecc 
			INNER JOIN emailcampaign ec ON ec.id = ecc.campaignId 
			INNER JOIN users u ON u.userID = ecc.userId
			INNER JOIN brokerages b ON u.brokerageID = b.brokerageID " ;
        

        $where = false;
        
        if( isset($_POST['columns'][4]['search']['value']) && strlen($_POST['columns'][4]['search']['value'])){
            $brokerageId = intval($_POST['columns'][4]['search']['value']);
        ...
        if( isset($_POST['columns'][3]['search']['value']) && strlen($_POST['columns'][3]['search']['value'])){
            if( !$where ){
                $query .= " WHERE ec.title LIKE '%" . $_POST['columns'][3]['search']['value'] . "%' ";
            }else{
                $query .= " AND ec.title LIKE '%" . $_POST['columns'][3]['search']['value'] . "%' ";
            }
            $where = true;
        }
        ...
        */
        ...
        if( isset($_POST['columns'][5]['search']['value']) && strlen($_POST['columns'][5]['search']['value']) ){
            if( !$where ){
                $query .= " WHERE u.city LIKE '%" . $_POST['columns'][5]['search']['value'] . "%' ";
            }else{
                $query .= " AND u.city LIKE '%" . $_POST['columns'][5]['search']['value'] . "%' ";
            }
            $where = true;
        }

        if( isset($_POST['columns'][6]['search']['value']) && strlen($_POST['columns'][6]['search']['value']) ){
            if( !$where ){
                $query .= " WHERE u.state = '" . $_POST['columns'][6]['search']['value'] . "' ";
            }else{
                $query .= " AND u.state = '" . $_POST['columns'][6]['search']['value'] . "' ";
            }
            $where = true;
        ...
        $final = array();
        $first = true;
        file_put_contents(dirname(__FILE__) . "/ec-queries",date("Y-m-d H:i:s") . "::$query\n",FILE_APPEND);
...
$(document).ready(function(){
    table = $('#myTable').DataTable({
			"serverSide": true,
			"ajax": {
				'url': "/admin/email-campaign-leads.php",
				'type': 'POST'
			},
			"iDisplayLength": 50
		}
