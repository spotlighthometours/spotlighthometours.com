...
**********************************************************************************************/
	...
	if(isset($_GET['country']) && strlen($_GET['country']) > 0) {
		...
		$query1 .= '
			...
			FROM nf_pricing p
			LEFT JOIN nf_locations l ON p.categoryID = l.zipID
			...
			AND l.country = "' . CleanQuery($_GET['country']) . '"
		';
	
		if(isset($_GET['state']) && strlen($_GET['state']) > 0) {
			echo ' - ';
			echo $_GET['state'];
			$query1 .= 'AND l.state_prefix = "' . CleanQuery($_GET['state']) . '" ';
		}
		if(isset($_GET['county']) && strlen($_GET['county']) > 0) {
			echo ' - ';
			echo $_GET['county'];
			$query1 .= 'AND l.county = "' . CleanQuery($_GET['county']) . '" ';
		}
		if(isset($_GET['city']) && strlen($_GET['city']) > 0) {
			echo ' - ';
			echo $_GET['city'];
			$query1 .= 'AND l.city = "' . CleanQuery($_GET['city']) . '" ';
		}
		if(isset($_GET['zip']) && strlen($_GET['zip']) > 0) {
			echo ' - ';
			echo $_GET['zip'];
			$query1 .= 'AND l.zip_code = "' . CleanQuery($_GET['zip']) . '" ';
		}
		if(isset($_GET['country'])) {
			//$query1 .= 'LIMIT 1000';
		}
...
**********************************************************************************************/
		...
		if(isset($_GET['state']) && strlen($_GET['state']) > 0) {
			echo ' - ';
			echo $_GET['state'];
			$query .= 'AND l.state_prefix = "' . CleanQuery($_GET['state']) . '" ';
		}
		if(isset($_GET['county']) && strlen($_GET['county']) > 0) {
			echo ' - ';
			echo $_GET['county'];
			$query .= 'AND l.county = "' . CleanQuery($_GET['county']) . '" ';
		}
		if(isset($_GET['city']) && strlen($_GET['city']) > 0) {
			echo ' - ';
			echo $_GET['city'];
			$query .= 'AND l.city = "' . CleanQuery($_GET['city']) . '" ';
		}
		if(isset($_GET['zip']) && strlen($_GET['zip']) > 0) {
			echo ' - ';
			echo $_GET['zip'];
			$query .= 'AND l.zip_code = "' . CleanQuery($_GET['zip']) . '" ';
		}
		if(isset($_GET['country'])) {
			//$query .= 'LIMIT 1000';
		}
...
**********************************************************************************************/	
		if(isset($_GET['state'])&&strlen($_GET['state']) > 0){
			...
			$query = '
				...
				LEFT JOIN nf_locations l ON bl.categoryID = l.zipID
