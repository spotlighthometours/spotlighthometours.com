...
if( isset($_POST['submit']) && $_POST['submit'] == 'Create' ){
	...
	$fixes->newNote(
		intval($_POST['tourId']), 			//Tour ID
		intval($_SESSION['admin_id']),		//OP ID
		null,								//Editor ID
		htmlentities($_POST['notes']),		//Notes
		($parent===null ? "employee" : "editor" ),	//Employee type
		$fileKey,							//File key
		$parent,
        $fixType,
		$urgency
...
<div id='results_wrapper'>
        ...
        <a href='#heatherizedStart'>Go to Heatherized</a>
	</div>
<div id="tableWrapper" class="visible" >
	...
	<?php
        ...
        if( isset($_GET['searchByTourId']) ){
        	dumpTable(" parentId IS NULL and status='" . fixes::OPEN . "' AND tourId=" . $_GET['searchByTourId'], /* Query */
				$_GET['searchByTourId'], /* Tour ID */
				false, /* resolved */
				true, /* Oldest first */
				20 /* LIMIT */
        	);
        	echo "<div style='padding-top: 50px;'><h1>Resolved: </h1><hr></div>";
        	dumpTable(/*" parentId IS NULL and */ " status='" . fixes::RESOLVED . "' AND tourId=" . $_GET['searchByTourId'], /* query */
			null, /* Search by tour ID */
			true, /* Resolved */
			false, /* Oldest First */
			20 /* LIMIT */
        	);
        	...
        	dumpTable(" parentId IS NULL AND status='" . fixes::OPEN . "'", /* query */
				null, /* Tour ID */
				false, /* Resolved */
				true, /* Oldest first */
				20 /* LIMIT */
				);
            echo "<a id='heatherizedStart'></a>";
            echo "<h2>Needs Heatherization</h2>";
            dumpTable(" parentId IS NULL AND heatherized != 1 AND status='resolved' ",$_tourId = null,$resolved=true,$oldestFirst=true,$heatherMode=false);
        	echo "<h2>Resolved and Heatherized: </h2><hr>";
        	dumpTable(
        	   " parentId IS NULL AND status='" . fixes::RESOLVED . "' AND heatherized != 0 ", /* Query */
        	   null, /* Tour ID */
        	   true, /* resolved */
        	   false, /* Oldest first */
			   20 /* LIMIT */
        ...
        function dumpTable($query,$_tourId = null,$resolved=false,$oldestFirst=true,$heatherMode=false,$limit=50){
                global $fixes;
                global $db;
                $tour = new \tours();
                $rows = $fixes->getList($query,$limit);
                ...
                foreach($rows as $array => $item){
					...
					if( $item['urgent'] == '1' ){
	                    echo '<tr class="urgent" id=trRows_' . $item['id'];
					}else{
