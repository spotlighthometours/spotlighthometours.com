...
<div class="filtering">
	<div class="templates">
        <form>
            <label>Show tours that are: 
                <select name="filterTemplate">
               	  <option value="0">Tours (all)</option>
			...
			$('#videoReportContainer').jtable({
				...
				fields: {
					...
					tpTourTypeID: {
						...
						type: 'checkbox',
						display: function (data) {
        					var tourTypeNames = Array();
							<?PHP $firstTT = true; foreach($tourtypelist as $row => $columns){?>tourTypeNames[<?PHP echo $columns['tourTypeID'] ?>]='<?PHP echo str_replace("'","",$columns['tourTypeName']) ?>';<?PHP echo "\n";} ?>
							if(data.record.tpTourTypeID==0){
								return tourTypeNames[data.record.tourTypeID];
							}else{
								return tourTypeNames[data.record.tpTourTypeID];
							}
    					},
						width: '7%'
					},
					...
					tourSheetLink:{
						...
						display: function (data) {
							if(data.record.tpTourTypeID==0){
								...
								return '<a href="http://www.spotlighthometours.com/admin/users/users.cfm?pg=toursheet&tour='+data.record.tourid+'&tourt='+data.record.tpTourTypeID+'&user='+data.record.userID+'" target="_new">tour sheet</a>';
							}
						},
						width: '5%'
					}
